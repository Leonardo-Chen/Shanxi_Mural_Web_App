...
export function locMural(locale: Locale, mural: Mural): Mural {
  ...
  return {
    ...
    displayTitle: locPair(locale, mural.displayTitle, o?.displayTitle),
    templeName: locMuralTemple(locale, mural.templeName),
    hall: mural.hall ? locHall(locale, mural.hall) : mural.hall,
    period: mural.period ? locEra(locale, mural.period) : mural.period,
    alt: locPair(locale, mural.alt, o?.displayTitle),
    description: locPair(locale, mural.description ?? "", o?.summary) || mural.description,
    detailedDescription:
      locPair(locale, mural.detailedDescription ?? "", o?.detailedDescription) ||
      mural.detailedDescription,
    readingGuide: mural.readingGuide?.map((step, i) =>
      locPair(locale, step, o?.readingGuide?.[i])
    ),
    location: locPair(locale, mural.location ?? "", o?.location) || mural.location,
    locationPrecision:
      locPair(locale, mural.locationPrecision ?? "", o?.locationPrecision) ||
...
export function locFigure(locale: Locale, figure: Figure) {
  ...
  const zhName = figure.displayName ?? figure.name;
  const zhDesc = figure.shortDescription ?? figure.note ?? figure.introduction;
  return {
    displayName: locPair(locale, zhName, o?.name),
    researchName: locPair(locale, figure.researchName ?? zhName, o?.name),
